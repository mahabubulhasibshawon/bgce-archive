...
ক্লোজার (Closure) হলো এমন একটি ফাংশন, **যা অন্য একটি ফাংশনের ভিতরে define করা হয়** এবং **যা তার নিজের স্কোপ ছাড়াও তার আউটার (বাইরের) স্কোপে** থাকা **ভেরিয়েবলগুলোকে** মনে রাখে এবং ব্যবহার করতে পারে, এমনকি সেই আউটার স্কোপটি execute হওয়া শেষ হয়ে গেলেও।
...
* `money` ভেরিয়েবলটি inner function দ্বারা capture করা হয়।
* প্রতিবার call করলে `money` আপডেট হয়।
...
## 🔍 Closures এর প্রকারভেদ

### 1. **Closure with Outer Variable**

**প্রশ্ন:** একটি Go প্রোগ্রাম লিখুন যা দেখায় কীভাবে closure outer function থেকে variable access ও modify করতে পারে।
...
* `outer` ফাংশন একটি closure তৈরি করে যা `x` ভেরিয়েবল capture করে এবং modify করে।
...
**প্রশ্ন:** দেখান কীভাবে একই ফাংশনে তৈরি হওয়া একাধিক closures তাদের নিজস্ব স্টেট ধরে রাখে।
...
* `counter1` এবং `counter2` প্রতিটিই আলাদা closure, যাদের নিজস্ব `counter` স্টেট রয়েছে।
* এরা একে অপরকে প্রভাবিত করে না।
...
**প্রশ্ন:** এমন একটি closure লিখুন যা parameter গ্রহণ করে এবং দেখায় যে closures কীভাবে arguments ব্যবহার করতে পারে।
...
* `multiplier` নামের closure `factor` প্যারামিটার গ্রহণ করে এবং একটি ফাংশন রিটার্ন করে যা `n` কে `factor` দিয়ে গুণ করে।
...
**প্রশ্ন:** Go-তে closures কীভাবে deferred execution এর সঙ্গে ব্যবহার করা যায় এবং outer function শেষ হওয়ার পর ভেরিয়েবল access করলে কী ঘটে?
...
func main() {
    a := 10
    defer func(a int) { // 'a' প্যারামিটার হিসেবে পাঠানো হয়েছে
        fmt.Println("Deferred closure:", a)
    }(a) // এখানে a এর মান পাস করা হয়েছে
    a = 20
...
* যদিও `main` এ `a` পরিবর্তিত হয়েছে, deferred closure-এ `a` এর যে মান পাঠানো হয়েছে সেটাই প্রিন্ট হবে।
* কারণ `a` প্যারামিটার হিসেবে capture করা হয়েছে, reference নয়।

---

### 5. **Closure Capturing Loop Variable**

**প্রশ্ন:** একটি Go প্রোগ্রাম লিখুন যা দেখায় কীভাবে closure loop variable ভুলভাবে capture করে।
...
func main() {
    ...
    for i := 0; i < 3; i++ {
        i := i // প্রতি iteration-এ নতুন i তৈরি হচ্ছে
        funcs = append(funcs, func() {
...
* `adder` ফাংশন `a` কে capture করে এবং `b` এর সাথে যোগ করে।
...
**প্রশ্ন:** একটি closure তৈরি করুন যা function factory হিসেবে কাজ করে এবং পাস করা argument অনুসারে বিভিন্ন mathematical operation রিটার্ন করে।
...
* `operationFactory` পাস করা operator অনুযায়ী একটি closure রিটার্ন করে।
...
**প্রশ্ন:** এমন একটি closure লিখুন যা বারবার call করার পরও তার স্টেট সংরক্ষণ করে (যেমন একটি simple counter) এবং এর কার্যকারিতা ব্যাখ্যা করুন।
...
* `c1` এবং `c2` আলাদা আলাদা স্টেট সংরক্ষণ করে।

---

### 9. **Closure with Function Composition**

**প্রশ্ন:** একটি Go প্রোগ্রাম তৈরি করুন যা closures ব্যবহার করে function composition প্রদর্শন করে।
...
* `compose` ফাংশন দুটি ফাংশন `f` এবং `g` গ্রহণ করে এবং একটি নতুন ফাংশন রিটার্ন করে যা `g(x)` এর ওপর `f()` apply করে।
* এখানে `double(addFive(3))` => `double(8)` => `16`

---
# Go Closures - কোড উদাহরণ ও ব্যাখ্যাসহ ২০টি প্রশ্ন

এই ডকুমেন্টে Go-এর closures নিয়ে ২০টি প্রশ্ন, কোড উদাহরণ এবং বিস্তারিত ব্যাখ্যা রয়েছে।

---

### 1. **Go-তে closure কী?**

**প্রশ্ন:** Go-এ closure কী তা সংজ্ঞায়িত করুন একটি উদাহরণসহ।
...
### 2. **একটি closure কীভাবে বাইরের function-এর variable access করে?**

**প্রশ্ন:** দেখান কিভাবে একটি closure বাইরের variable access ও modify করতে পারে।
...
**ব্যাখ্যা:**  
এই closure বাইরের `x` variable ধরে রাখে এবং প্রতিবার call করার সময় তাকে modify করে।
...
**প্রশ্ন:** একটি closure লিখুন যেটি একটি parameter নেয় এবং দেখান এটি কিভাবে কাজ করে।
...
### 9. **closure যেটি তার পূর্ববর্তী অবস্থা মনে রাখে**
...
**ব্যাখ্যা:**  
anonymous function closure হিসেবে কাজ করতে পারে। এখানে `a` ভ্যারিয়েবলটি capture করে রেখেছে function টা।
...
**ব্যাখ্যা:**  
এই উদাহরণে, `lazySum` ফাংশনটি actual calculation defer করে রাখে যতক্ষণ না `sum()` call করা হয়। এটা lazy evaluation এর একটি সাধারণ রূপ।
...
**ব্যাখ্যা:**  
`filter` ফাংশনটি একটি closure নেয় যেটি প্রতিটি item evaluate করে। এখানে, `even` একটি closure যা শুধু জোড় সংখ্যা বেছে নেয়।
...
**ব্যাখ্যা:**  
এই closure একটি map-এর মাধ্যমে আগে হিসাব করা ফলাফল মনে রাখে। একই input দিলে সে পুরোনো রেজাল্ট ব্যবহার করে।
...
**ব্যাখ্যা:**  
closure সেই scope-এর variable ধরে যেখানে এটি define হয়েছে। এখানে closure `x := 10` এর ভ্যালু ধরে রেখেছে।

---

### 17. **Closure-এ pointer capture করা**

**প্রশ্ন:** কিভাবে closure pointer capture করে এবং সেটার প্রভাব কী?
